	...
	function setup()
	{
		...
		for(var i=0; i<20; i++)
		{
			for(var j=0; j< 10; j++)
			{
				...
				var targetPos = m.position.clone();

				var offset = randf(0,2000);
				m.position.y += 3000 + offset;

				new TWEEN.Tween(m.position)
					.to(targetPos, 2000 + offset)
					.easing( TWEEN.Easing.Quadratic.InOut )
					.onUpdate(function( u ){

						var u2pi = cos(u * TWO_PI);
						this.rotation.z = lerp(m.position.y * .0005, 0, u2pi);
						this.rotation.x = lerp(m.position.y * .001, 0, u2pi);

						var scl = lerp(10, 100, Math.pow(u2pi, 2.));
						var otherScl = lerp( scl, 300, 1 - u2pi);

						this.scale.set(otherScl, scl, otherScl);
						// this.rotation.y = lerp(m.position.y * .002, 0, cos(u * TWO_PI));
					}.bind( m ))
					.repeat( 1000 )
					.yoyo( true )
					.delay( 2000 )
