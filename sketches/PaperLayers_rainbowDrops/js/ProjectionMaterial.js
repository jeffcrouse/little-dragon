...
var ProjectionMaterial = function(params)
{
	...
	var matParams = {
		...
		'	uv += vec2( getNoise( vec3(texCoordProj.xz * .03, time + uv.y + texCoordProj.y * .005) ) * .25, 0.);',

		'	gl_FragColor = texture2D( colorRamp, uv ) * .75 + .3;',
		...
		'	float dropShadow = clamp( (1. - mod(uv.x, rampStep) / rampStep) * 7.5, 0., 1.);',

		'	gl_FragColor.xyz *= mapLinear( dropShadow, 0., 1., .925, 1.);',
